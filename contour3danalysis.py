...
def analyze(filter):
    ...
    for xx in range(len(X)):
        ...
        Z += [z_]
    Z = np.array(Z)
    cset = ax.contourf(X, Y, Z, 3)
    ax.clabel(cset, fontsize=3, inline=2)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print "Usage: contour3danalysis.py <file> <street1> <street2> <street3> ..."
        sys.exit(1)
    plt = analyze(Filter(*sys.argv[1:]))
    plt.savefig("results/data.contour3d."+(".".join(sys.argv[2:])).replace(" ","_")+".png")
    print "0"